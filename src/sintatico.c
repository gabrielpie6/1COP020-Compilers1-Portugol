...
void deriveProduction(int production)
{
    ...
    switch(production - 100)
    {
        ...
        case  6: eat(FUNCAO); eat(IDENTIFICADOR); F(PARAMETROS); eat(DOIS_PONTOS); F(TIPOBASICO); eat(PONTO_E_VIRGULA); F(DECLARAPARAMETROS); F(BLOCOVARIAVEIS); F(BLOCOCOMANDOS); eat(PONTO_E_VIRGULA); break;
